...
export default class App extends Component {
  ...
  getYoutube(varAPIKey, previousPage, nextPage) {
    fetch("https://www.googleapis.com/youtube/v3/search?key=" + varAPIKey + nextPage + previousPage + "&channelId=UCX-4mrOc5r691SzDhHtkOgw&part=snippet,id&order=date", {
      method: 'GET',
      dataType: 'json',
    }).then(
      function (response) {
  ...
  getFullYoutube(varAPIKey) {
    if (this.state.nextPage1 !== '&pageToken=' + undefined) {
      fetch("https://www.googleapis.com/youtube/v3/search?key=" + varAPIKey + this.state.nextPage1 + "&channelId=UCX-4mrOc5r691SzDhHtkOgw&part=snippet,id&order=date", {
        method: 'GET',
        dataType: 'json',
      }).then(
        function (response) {
  ...
  render() {
    return (
      <View style={styles.container}>
        <HeaderMenu pagetitle='YUFID TV' openMenu={this._onMenu} openSearch={this._onSearch} />
