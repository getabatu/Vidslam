...
export default class App extends Component {
  ...
  getYoutube(varAPIKey, previousPage, nextPage) {
    fetch("https://www.googleapis.com/youtube/v3/search?key=" + varAPIKey + nextPage + previousPage + "&channelId=UCgCksPJCnRz4JCAgYKs32kA&part=snippet,id&order=date", {
      method: 'GET',
      dataType: 'json',
    }).then(
      function (response) {
  ...
  getFullYoutube(varAPIKey) {
    if (this.state.nextPage1 !== '&pageToken=' + undefined) {
      fetch("https://www.googleapis.com/youtube/v3/search?key=" + varAPIKey + this.state.nextPage1 + "&channelId=UCgCksPJCnRz4JCAgYKs32kA&part=snippet,id&order=date", {
        method: 'GET',
        dataType: 'json',
      }).then(
        function (response) {
    ...
    setTimeout(() => {
      return (
        <View style={styles.viewEmptyData}>
          ...
          <Text ncote style={[{ marginTop: 10 }]}>There's no Videos</Text>
  ...
  render() {
    return (
      <View style={styles.container}>
        <HeaderMenu pagetitle='LENTERA ISLAM' openMenu={this._onMenu} openSearch={this._onSearch} />
